...
)
{
   ...
   fprintf(fp, "lb_y = %15.8e\n", lb_y);
   fprintf(fp, "ub_y = %15.8e\n", ub_y);
   fprintf(fp, "lb_u = %15.8e\n", lb_u);
   fprintf(fp, "ub_u = %15.8e\n", ub_u);
   fprintf(fp, "u_init = %15.8e\n", u_init);

   for (Index i=0; i<=N_+1; i++) {
      for (Index j=0; j<=N_+1; j++) {
         fprintf(fp, "y[%6d,%6d] = %15.8e\n", i, j, y_d_[y_index(i,j)]);
      }
   }
   /*
   for (Index j=1; j<=N_; j++) {
      fprintf(fp, "u[%6d,%6d] = %15.8e\n", 0, j, y_d_[u0j_index(j)]);
   }
   for (Index j=1; j<=N_; j++) {
      fprintf(fp, "u[%6d,%6d] = %15.8e\n", N_+1, j, y_d_[u1j_index(j)]);
   }
   for (Index i=1; i<=N_; i++) {
      fprintf(fp, "u[%6d,%6d] = %15.8e\n", i, 0, y_d_[ui0_index(i)]);
   }
   for (Index i=1; i<=N_; i++) {
      fprintf(fp, "u[%6d,%6d] = %15.8e\n", i, N_+1, y_d_[ui1_index(i)]);
   }
   */
